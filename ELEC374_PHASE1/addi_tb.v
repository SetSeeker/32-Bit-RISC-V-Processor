...
always @(posedge Clock) begin
	case (Present_state)
		...
		T1 : Present_state = T2;
		T2 : Present_state = T3;
		T3 : Present_state = T4_add; // Change to T4_and or T4_or for andi/ori
		T4_add, T4_and, T4_or : Present_state = T5;
	endcase
end
...
always @(Present_state) begin
	case (Present_state)
		...
		T4_and: begin // AND Immediate (andi)
			Cout <= 1; control <= 5'd8; // AND control signal
			#5 Zin <= 1;
			#15 Zin <= 0; Cout <= 0;
		end
		T4_or: begin // OR Immediate (ori)
			Cout <= 1; control <= 5'd9; // OR control signal
			#5 Zin <= 1;
			#15 Zin <= 0; Cout <= 0;
		end
